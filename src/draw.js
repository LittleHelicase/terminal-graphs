...
function angleDir (angle) {
  ...
  if (shortAngle > -0.3 && shortAngle < 0.3) return 'horizontal'
  else if (shortAngle >= 0.3 && shortAngle < 1.3) return 'down-diagonal'
  else if (shortAngle <= -0.3 && shortAngle > -1.3) return 'up-diagonal'
...
function combine (ops, opt) {
  if (ops.length > 0 && typeof(ops[0]) !== 'function' && !Array.isArray(ops[0]))
    throw new Error('Cannot combine non functions or arrays')
  return (field) => {
...
function drawBox (box) {
  if (box.id === 'root') return (x) => x
  else return combine([
    drawLine(leftUpper(box), leftLower(box)),
    drawLine(rightUpper(box), rightLower(box)),
    drawLine(leftUpper(box), rightUpper(box)),
    drawLine(leftLower(box), rightLower(box)),
    setCharacter(leftUpper(box), '┌'),
    setCharacter(leftLower(box), '└'),
    setCharacter(rightUpper(box), '┐'),
    setCharacter(rightLower(box), '┘'),
    drawLabel(box)
  ])
}
...
const triples = (callback, sequence) => {
  return sequence.reduce((acc, cur) => {
    if (acc[0] == null) return [[cur], acc[1]]
    else if (acc[0].length === 1) return [[acc[0][0], cur], acc[1]]
    else {
      var fst = acc[0][0]
      var sec = acc[0][1]
...
function corner (from, mid, end) {
  ...
  if ((dir1 === 'horizontal' || dir1 === 'vertical') && (dir2 === 'vertical' || dir2 === 'horizontal')) {
    if (from.y < mid.y) { // first downwards
      if (from.x < end.x) return 'left-lower'
      else return 'right-lower'
    } else if (from.y > mid.y) { // first upwards
      if (from.x < end.x) return 'right-upper'
      else return 'left-upper'
    } else if (from.x < mid.x) { // first right
      if (from.y < end.y) return 'right-upper'
      else return 'right-lower'
    } else if (from.x > mid.x) { // first left
...
function drawEdge (edge) {
  ...
  return combine(betweenPairs((a, b) => drawLine(a, b), pts)
    .concat(
      triples((from, mid, end) => {
